...
const RUTA_EXCEL_DEFAULT = "/data/excel/avances_proyectos.xlsx";
...
async function cargarSecretariasDesdeExcel(opciones = {}) {
    ...
    try {
        ...
        const secretarias = adaptarDatosSecretarias(datosPorSecretaria, metadata)

        return enriquecerSecretariasConMetadata(secretarias, metadata)
    } catch (error) {
...
function enriquecerSecretariasConMetadata(secretarias = [], metadata = []) {
    const listaMetadata = normalizarListaMetadata(metadata)

    return secretarias
        .map((secretaria, indice) => {
            const metadataEncontrada = buscarMetadataServicio(secretaria, listaMetadata)

            return {
                ...secretaria,
                id: metadataEncontrada?.id || secretaria.id || normalizarIdBusqueda(secretaria.sigla),
                sigla: metadataEncontrada?.sigla || secretaria.sigla || secretaria.id,
                nombre: metadataEncontrada?.nombre || secretaria.nombre || secretaria.sigla,
                titular: metadataEncontrada?.titular || secretaria.titular || 'Por capturar',
                color: metadataEncontrada?.color || secretaria.color || '#9f2241',
                orden: metadataEncontrada?.orden ?? secretaria.orden ?? indice + 1,
            }
        })
        .sort((a, b) => {
...
function buscarMetadataServicio(secretaria = {}, listaMetadata = []) {
    const candidatosSecretaria = [
        ...
        .map(normalizarClaveSecretariaServicio)
        .filter(Boolean)

    return listaMetadata.find((metadata) => {
...
function normalizarClaveSecretariaServicio(valor) {
    return String(valor || '')
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
        .toLowerCase()
